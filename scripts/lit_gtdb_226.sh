...
set -euo pipefail
mkdir -p /shares/sander.imm.uzh/MM/kansasii/lit/gtdb/gtdb226
cd /shares/sander.imm.uzh/MM/kansasii/lit/gtdb/gtdb226
...
if [ ! -f bac120_metadata_r226.tsv ]; then
  wget https://data.gtdb.aau.ecogenomic.org/releases/release226/226.0/bac120_metadata_r226.tsv.gz
  gunzip bac120_metadata_r226.tsv.gz
fi
if [ ! -f bac120_taxonomy_r226.tsv ]; then
  wget https://data.gtdb.aau.ecogenomic.org/releases/release226/226.0/bac120_taxonomy_r226.tsv.gz
  gunzip bac120_taxonomy_r226.tsv.gz
fi
...
grep -E "$SPECIES_PATTERN" bac120_metadata_r226.tsv > kansasii_complex_rows_metadata.tsv
grep -E "$SPECIES_PATTERN" bac120_taxonomy_r226.tsv > kansasii_complex_rows_taxonomy.tsv
...
GTDBTYPE_COL=$(head -1 bac120_metadata_r226.tsv | tr '\t' '\n' | grep -n '^gtdb_type_designation_ncbi_taxa$' | cut -d: -f1)
NCBITYPE_COL=$(head -1 bac120_metadata_r226.tsv | tr '\t' '\n' | grep -n '^ncbi_type_material_designation$' | cut -d: -f1)
...
BIOSAMPLE_COL=$(head -1 bac120_metadata_r226.tsv | tr '\t' '\n' | grep -n '^ncbi_biosample$' | cut -d: -f1)
...
TAX_COL=$(head -1 bac120_metadata_r226.tsv | tr '\t' '\n' | grep -n '^gtdb_taxonomy$' | cut -d: -f1)
REPR_COL=$(head -1 bac120_metadata_r226.tsv | tr '\t' '\n' | grep -n '^gtdb_genome_representative$' | cut -d: -f1)
ASSEMNAME_COL=$(head -1 bac120_metadata_r226.tsv | tr '\t' '\n' | grep -n '^ncbi_assembly_name$' | cut -d: -f1)
